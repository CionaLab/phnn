...
# Kruskal-Wallis test on swim duration
df_1 %>% kruskal_test(swim_time ~ group)

# run Dunn's test if Kruskal-Wallis test shows significance;
# then save results to a csv
df_1 %>%
  filter(swim_time != 0) %>%
  dunn_test(swim_time ~ group) %>%
  write_csv("data.csv")
...
ggplot(
  df_1,
  aes(x = group, y = swim_time, fill = group)
) +
  geom_violin() +
...
df_1 %>%
  group_by(group) %>%
  summarise(mean = mean(swim_time), sd = sd(swim_time)) %>%
  write_csv("data.csv")
